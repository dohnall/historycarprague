...
class Section {
	...
	private function setSectionText($newdata, $new_version) {
		...
		if($new_version === false && $this->hasVersion()) {
			...
			$query = "INSERT INTO ".Config::db_table_section_text()."
					  ...
					  VALUES
					  (".$this->section_id.", ".$this->lang_id.", ".$this->session->user_id.", '".mysql_real_escape_string($data['name'])."',
					   '".$this->makeUrl($data['url'], $data['name'])."', '".mysql_real_escape_string($data['url_children'])."', '".mysql_real_escape_string($data['title'])."', '".mysql_real_escape_string($data['title_children'])."',
					   '".mysql_real_escape_string($data['description'])."', '".mysql_real_escape_string($data['keywords'])."', '".$data['views']."', NOW(), NOW(), '".$data['status']."')";
					...
					if(is_array($value) && $field != 'blob') {
						foreach($value as $v) {
							...
							if(isset($col['param']['relation']) && $col['param']['relation']) {
								...
								$query = "INSERT INTO ".Config::db_table_section_text_value()."
										  ...
										  VALUES
										  (".$section->get('text', 'section_text_id').", '".$col['param']['relation']."', '".mysql_real_escape_string($this->section_id)."')";
								$this->db->insert($query);
							} else {
								$query = "INSERT INTO ".Config::db_table_section_text_value()."
										  (section_text_id, code, ".$field."_val)
										  VALUES
										  (".$this->get('text', 'section_text_id').", '".$col['item']['code']."', '".mysql_real_escape_string($v)."')";
								$this->db->insert($query);
							}
						}
					} else {
						...
						if(isset($col['param']['relation']) && $col['param']['relation']) {
							...
							$query = "INSERT INTO ".Config::db_table_section_text_value()."
									  (section_text_id, code, ".$field."_val)
									  VALUES
									  (".$this->get('text', 'section_text_id').", '".$col['item']['code']."', '".mysql_real_escape_string($value)."')";
							$this->db->insert($query);
						}
					}
				}
			...
			foreach($value['file'] as $k => $file) {
				if(!empty($file)) {
					$query = "INSERT INTO ".Config::db_table_section_file()."
							  (section_text_id, file, code, alt, description, download, rank)
							  VALUES
							  (".$this->get('text', 'section_text_id').", '".$file."', '".$code."', '".$value['alt'][$k]."', '".$value['description'][$k]."', '".$value['download'][$k]."', ".($rank++).")";
					$this->db->insert($query);
				}
			}
		}
    ...
    private function loadPath() {
        ...
        for($i = 2; $i <= $this->get('section', 'depth'); $i++) {
            $j = $i - 1;
            $query.= " LEFT JOIN ".Config::db_table_tree()." t".$i." ON (t".$i.".section_id = t".$j.".parent_id)";
        }
    ...
    private function loadUrl($ajax = false) {
        ...
        $query = "SELECT st1.section_id, st1.url, st1.url_children
                  ...
                  WHERE st1.section_text_id = (
					        ...
					        WHERE st2.section_id = st1.section_id AND
					        	  st2.lang_id = st1.lang_id AND
								  t2.main = '1') AND
						s1.section_id IN (".implode(', ', array_keys($this->get('path'))).") AND
						st1.lang_id = ".$this->lang_id." AND
                        t1.main = '1'
                  ORDER BY depth ASC";
        $result = $this->db->select($query);
        ...
        if($this->lang_id != $this->session->default_lang_id) {
