...
class SectionList {
	...
	public function getSectionsByTemplate($template, $type=1, $parent_id=0) {
	...
	public function getSectionsByUrl($url) {
    ...
    public function import($parent_id, $template, $data) {
        ...
        foreach($data as $k => $row) {
        	if($k) {
				...
				$data['text']['status'] = $cols[1];
				foreach($arr as $k => $col) {
					$value = explode('|||', $cols[$k+2]);
					if(in_array($col['item']['type'], array('file', 'image'))) {
						foreach($value as $v) {
							$data['file'][$col['item']['code']]['file'][] = $v;
							$data['file'][$col['item']['code']]['alt'][] = '';
							$data['file'][$col['item']['code']]['description'][] = '';
						}
					} else {
						foreach($value as $v) {
							$data['value'][$col['item']['code']][] = $v;
						}
					}
				}
				$section->save($data);
