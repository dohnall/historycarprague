...
class CodelistRecord {
        ...
        if($this->id) {
			...
			foreach($data['name'] as $lang_id => $name) {
	            ...
	            $query = "INSERT INTO ".Config::db_table_codelist_record()."
	                      ...
	                      VALUES
	                      ('".$codelist_text_id."', '".mysql_real_escape_string($data['code'])."', '".mysql_real_escape_string($name)."', ".$rank.", NOW())";
				...
				if(is_array($value)) {
					foreach($value as $v) {
						...
						$query = "INSERT INTO ".Config::db_table_codelist_record_value()."
								  ...
								  VALUES
								  (".$this->id.", '".mysql_real_escape_string($col['item']['code'])."', '".mysql_real_escape_string($v)."')";
					...
					$query = "INSERT INTO ".Config::db_table_codelist_record_value()."
							  ...
							  VALUES
							  (".$this->id.", '".mysql_real_escape_string($col['item']['code'])."', '".mysql_real_escape_string($value)."')";
					$this->db->insert($query);
				}
			}
		}
			...
			foreach($value['file'] as $k => $file) {
				if(!empty($file)) {
					$query = "INSERT INTO ".Config::db_table_codelist_record_file()."
							  (codelist_record_id, file, code, alt, description, rank)
							  VALUES
							  (".$this->id.", '".$file."', '".$code."', '".$value['alt'][$k]."', '".$value['description'][$k]."', ".($rank++).")";
					$this->db->insert($query);
				}
			}
		}
	...
	private function loadFile() {
		...
		foreach($res as $file) {
			$this->data['file'][$file['code']]['file'][] = $file['file'];
			$this->data['file'][$file['code']]['alt'][] = $file['alt'];
			$this->data['file'][$file['code']]['description'][] = $file['description'];
		}
	}
