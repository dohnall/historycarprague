...
class WEB {
    ...
    public function show() {
        ...
        $this->smarty->assign(array(
            ...
            'ALERT_TYPE' => (!empty($this->session->alert_type) ? $this->session->alert_type : 'notice'),
        ));
	...
	private function setSection() {
		...
		//subpage
			...
			$sections = $sectionList->getSectionsByUrl($this->sectionUrl);
			foreach($sections as $section_id) {
				$section = Section::getInstance($section_id);
				$lang_code = $this->session->lang_id == $this->session->default_lang_id ? "" : $this->langCode."/";
                if($section->get('url') == ROOT.$lang_code.$this->path."/" && $section->isActive()) {
