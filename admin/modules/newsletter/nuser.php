...
if(isset($_POST['action']) && $_POST['action'] == "deleteItem") {
    $this->db->delete("DELETE FROM ".Config::db_table_nuser()." WHERE `nuser_id`='".$_POST['item_id']."'");
    $this->db->delete("DELETE FROM ".Config::db_table_nuser_ngroup()." WHERE `nuser_id`='".$_POST['item_id']."'");
    $this->db->delete("DELETE FROM ".Config::db_table_nqueue()." WHERE `nuser_id`='".$_POST['item_id']."'");
    Common::redirect();
...
if($this->session->nfilter) {
	...
	if(isset($filter['email']) && $filter['email']) {
		$where.= " AND u.email LIKE '%".mysql_real_escape_string($filter['email'])."%'";
	}
	if(isset($filter['fname']) && $filter['fname']) {
		$where.= " AND u.fname LIKE '%".mysql_real_escape_string($filter['fname'])."%'";
	}
	if(isset($filter['lname']) && $filter['lname']) {
		$where.= " AND u.lname LIKE '%".mysql_real_escape_string($filter['lname'])."%'";
	}
